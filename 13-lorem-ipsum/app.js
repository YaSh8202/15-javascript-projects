...
form.addEventListener('submit',function(e){
  e.preventDefault();//to prevent form to send the data to the action attribute
  //  console.log('hello');
  const value = parseInt(amount.value);//type of amount.value is string, using parseInt to converting to int
  // note string color is black and integer's is blue
  // if nothing is entered in number box value will be empty/NaN
  ...
  if(isNaN(value) || value< 0 || value> 9){
    ...
    result.innerHTML = ``;
    tempText.forEach((t)=>{
      result.innerHTML += `<p class="result">${t}</p>`;
    })
  }
})